    ...
    <form action="{{ route('admin.products.update', $product) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="w-full px-6">
                ...
                <div class="mt-6">
                    <div class="form-checkbox">
                        <input type="checkbox" name="has_delivery" id="delivery" {{ old('has_delivery') ?? $product->has_delivery ? 'checked' : '' }}>
                        <label for="delivery">{{ __('admin.products.fields.has_delivery') }}</label>
                    </div>

                    <div class="form-checkbox">
                        <input type="checkbox" name="has_pickup" id="pickup" {{ old('has_pickup') ?? $product->has_pickup ? 'checked' : '' }}>
                    ...
                    {{--
                    <conditional :approved="{{ old('has_pickup') ?? $product->has_pickup ? 1 : 0 }}" inline-template>
                        <fieldset>
                            <div class="form-checkbox">
                                <input
                                    type="checkbox"
                                    ...
                                    id="pickup"
                                    @change="toggle"
                                    {{ old('has_pickup') ?? $product->has_pickup ? 'checked' : '' }}
                                >
                                <label for="pickup">{{ __('admin.products.fields.has_pickup') }}</label>
                            </div>
        ...
        <div class="mt-10 flex items-center">
            ...
            <div class="form-checkbox ml-8 mb-0">
                <input type="checkbox" name="is_published" id="published" {{$product->is_published ? 'checked' : ''}}>
