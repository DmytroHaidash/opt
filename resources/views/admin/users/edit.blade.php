...
@section('content')
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="post" enctype="multipart/form-data">
        ...
        <regions-cities-select
            region="{{ old('region_id') ?? $user->city->region_id }}"
            city="{{ old('city_id') ?? $user->city->id }}"
        ></regions-cities-select>
        ...
        <div class="form-group">
            ...
            <select name="role" id="role" class="form-control" onchange="rolesInput()">
        ...
        <div {{$user->hasRole('carrier') ? '': 'class="hidden"'}} id="for-carrier">
                    ...
                    <fieldset v-show="current === '{{ $locale }}'">
                        <div class="form-group">
                            ...
                            <textarea
                                ...
                                id="carrier_description-{{ $locale }}"
                                class="editor"
                            >{{ old('carrier_description.' . $locale) ?? $user->getTranslation('carrier_description', $locale) }}</textarea>
                        </div>
                    </fieldset>
                @endforeach
            </locales>
            <div class="form-group">
                ...
                <input id="type_car" type="text" class="form-control{{ $errors->has('type_car') ? ' has-error' : '' }}"
                       name="type_car" value="{{ old('type_car') ?? $user->type_car }}">
            </div>
            <div class="form-group">
                ...
                <input id="brand_car" type="text" class="form-control{{ $errors->has('brand_car') ? ' has-error' : '' }}"
                       name="brand_car" value="{{ old('brand_car') ?? $user->brand_car}}">
            </div>
            <div class="flex flex-wrap -mx-2">
                <div class="form-group w-1/2 px-2">
                    ...
                    <input name="tonnage" id="tonnage" type="number" class="form-control" step="0.001" min="0.001"
                           value="{{ old('tonnage') ?? $user->tonnage }}">
                </div>
                <div class="form-group w-1/2 px-2">
                    ...
                    <input name="price_km" id="price_km" type="number" class="form-control" step="0.01" min="0.01"
                           value="{{ old('price_km') ?? $user->price_km}}">
                </div>
            </div>
            <div class="form-group">
                ...
                <select name="car_region" id="car_region" class="form-control">
                    ...
                    @foreach($regions as $region)
                        <option
                            value="{{ $region->id }}"
                            {{ $region === $user->car_region ? 'selected' : '' }}
                        >
                            {{ $region->name }}
                        </option>
                    @endforeach
            ...
            <div class="form-checkbox">
                <input type="checkbox" name="all_region" id="all_region" {{$user->all_region ? 'checked' : ''}} onchange="addWorkedRegion()">
                <label for="all_region">{{ __('auth.all_region') }}</label>
            </div>
            <div class="flex flex-wrap mt-4 mb-3 {{$user->all_region ? 'hidden' : ''}}" id="addWorkedRegion">
                <p class="mb-2">{{__('auth.worked_region')}}</p>

                @foreach($regions as $region)
                    <div class="form-checkbox">
                        <input type="checkbox" name="worked_region[]" id="worked_region-{{$region->id}}"
                               value="{{$region->id}}" {{in_array($region->id, $user->carrier_regions->pluck('id')->toArray()) ? 'checked' : '' }}>
            ...
            <media-upload
                ...
                :items="{{ json_encode($user->truck) }}"
            ></media-upload>
            <div class="form-checkbox mt-4">
                <input type="checkbox" name="published_carrier" id="published_carrier" {{$user->published_carrier ? 'checked' : ''}}>
                <label for="published_carrier">{{ __('auth.published_carrier') }}</label>
            </div>
        </div>
        <div class="form-group {{$user->hasRole('buyer') ? '': 'hidden'}} " id="for-buyer">
        ...
        <conditional :approved="{{ old('change_password') ? 1 : 0 }}" inline-template>
            <div class="mt-6">
                <fieldset v-if="isApproved">
                    <hr class="mb-4 border-0 border-b-2">
                    <div class="form-group">
                ...
                <div class="form-checkbox">
                    <input
                        type="checkbox" name="change_password"
                        id="published"
                        {{ old('change_password') ? 'checked' : '' }}
                        @change="toggle"
                    >
                    <label for="published">{{ __('admin.users.fields.change_password') }}</label>
                </div>
            </div>
        </conditional>
